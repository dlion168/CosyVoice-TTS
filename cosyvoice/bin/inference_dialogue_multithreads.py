...
def save_audio(tts_fn, tts_speeches, sample_rate=22050):
    """Helper function to save audio using torchaudio."""
    torchaudio.save(tts_fn, tts_speeches, sample_rate=sample_rate)

def process_dialogue_segment(dialogue, index, args, model, device, all_possible_references, configs):
    ...
    with torch.no_grad():
        for _, batch in enumerate(test_dataloader):
            ...
            tts_speeches = model_output['tts_speech']
            speechs[ref_mapping[utts[0]]].append(model_output['tts_speech'])

            yield tts_fn, tts_speeches
    ...
    yield merge_tts_fn, merged_speeches
...
def main():
    ...
    with open(args.dialogue, 'r', encoding='utf-8') as f:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = []
            for index, dialogue in tqdm(enumerate(inference_segment)):
                dialogue = json.loads(dialogue.strip())
                for tts_fn, tts_speeches in process_dialogue_segment(
                        dialogue, index, args, model, device, all_possible_references, configs):
                    if tts_fn is not None:
                        futures.append(executor.submit(save_audio, tts_fn, tts_speeches))

            # Optionally, wait for all tasks to complete.
            for future in as_completed(futures):
                future.result()  # To handle exceptions if any
        
    # mp.spawn(job, args=(max_workers, all_data, args, device, configs), nprocs=max_workers, join=True)
    # mp.spawn(job, args=(max_workers, all_data, args, device, configs), nprocs=max_workers)
    # steps = args.num_example // max_workers
    # with ThreadPoolExecutor(max_workers=max_workers) as executor:  # Use threads for parallel execution
    #     for i in range(max_workers):
    #         inference_segment = all_data[args.start_index + i*steps : args.start_index + (i+1)*steps]
    #         executor.submit(tts_inference, inference_segment, args, device, configs)
