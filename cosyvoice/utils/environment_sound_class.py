import json
import random
import re
import torch
import torchaudio
import torchaudio.transforms as transforms

class EnvironmentalSoundGenerator:
    def __init__(self, data_dir, metadata_filename, audio_subpath):
        self.data_dir = data_dir
        self.metadata_path = f"{data_dir}/{metadata_filename}"
        self.audio_path = f"{data_dir}/{audio_subpath}"
        
        with open(self.metadata_path, "r") as f:
            self.label_to_ids = json.load(f)
            print("Loaded metadata.json", self.label_to_ids)

    @staticmethod
    def parse_tag(tag):
        tag = tag[1:-1]
        pattern = r"([^\s]+)(\s+[\w]+=[^>]+)*"
        match = re.match(pattern, tag)

        if match:
            parsed_dict = {"name": match.group(1)}
            attributes = re.findall(r"(\w+)=([^\s>]+)", tag)
            for key, value in attributes:
                parsed_dict[key] = float(value) if value.replace('.', '', 1).isdigit() else value
            return parsed_dict
        return None

    @staticmethod
    def adjust_loudness(audio, ref_audio, db):
        ref_rms = ref_audio.abs().mean()
        rms = audio.abs().mean()
        gain = 10 ** ((ref_rms - rms) / 20)
        audio = audio * gain

        scaling_factor = 10 ** (db / 20)
        audio = audio * scaling_factor
        audio = audio.clamp(-1, 1)

        return audio

    def generate_environmental_sound(self, tag):
        config = self.parse_tag(tag)
        sound_list = self.label_to_ids.get(config["name"], [])
        if not sound_list:
            raise ValueError(f"Sound not found for tag: {config['name']}")
        audio_id = random.choice(sound_list)
        filepath = self.audio_path.format(audio_id=audio_id)
        waveform, sample_rate = torchaudio.load(filepath)
        new_sample_rate = 22050
        resampler = transforms.Resample(sample_rate, new_sample_rate)
        waveform = resampler(waveform)
        time_limit = 5
        audio_tensor = waveform[:, :new_sample_rate * time_limit]
        return audio_tensor

    def extract_environment_tag(self, text, matches):
        total_tag_length = 0
        for i, match in enumerate(matches):
            arguments = match[0][1:-1].split()

            matches[i] = {
                "tag": match[0],
                "type": "interleave" if match[0][0] == "[" else "background",
                "position": (match[1] - total_tag_length) / len(text),
                "audio": self.generate_environmental_sound(match[0])
            }

            for arg in arguments[1:]:
                key, value = arg.split("=")
                matches[i][key] = value
            total_tag_length += len(match[0])
        return matches

    @staticmethod
    def find_tags(text):
        pattern = r"(\[.*?\]|\<.*?\>)"
        matches = [(match.group(), match.start(), match.end()) for match in re.finditer(pattern, text)]
        text = re.sub(pattern, '', text)
        return text, matches

    def separate_text_and_sound(self, text):
        text, matches = self.find_tags(text)
        tags = self.extract_environment_tag(text, matches)
        return text, tags

    def combine_speech_and_environment(self, speech, tags):
        tag = tags[0]
        speech = speech[0]
        tag_audio = tag["audio"][0]
        tag_audio = self.adjust_loudness(audio=tag_audio, ref_audio=speech, db=float(tag["rel_db"]))
        position = int(speech.size(0) * tag["position"])

        if tag["type"] == "interleave":
            speech = torch.cat((speech[:position], tag_audio, speech[position:]))
        else:  # "background" type
            end = position + tag_audio.size(0)
            if end > speech.size(0):
                padding = torch.zeros(end - speech.size(0))
                speech = torch.cat((speech, padding))
            speech[position:end] += tag_audio

        return speech.unsqueeze(0)

if __name__ == "__main__":
    generator = EnvironmentalSoundGenerator(
        data_dir="/nfs/nas-6.1/cjtsao/environmental_sound/DataGeneration/humannoise",
        metadata_filename="metadata.json",
        audio_subpath="data/{audio_id}"
    )

    text, tags = generator.separate_text_and_sound("<Boiling rel_db=1> 有沒有聽到什麼聲音？")

    print("Text after removal:", text)
    print("Tags with indices:", tags)

    example_audio, sample_rate = torchaudio.load(
        "/nfs/nas-6.1/cjtsao/environmental_sound/CosyVoice-TTS/examples/libritts/cosyvoice/test_dir/000015_User_0.wav"
    )
    combined_audio = generator.combine_speech_and_environment(example_audio, tags)
    torchaudio.save("test_combined_audio.wav", combined_audio, sample_rate=22050)
